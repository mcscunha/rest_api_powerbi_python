...
dfInject = spark.read.format('http').json(
    'http://services.odata.org/V3/Northwind/Northwind.svc/Customers?$format=json')
dfInject = dfInject.select(
    "CostumerID", "CompanyName", "PostalCode", "City", "Phone", "Fax").limit(10)
...
pbi.executePBIOperation("postdataset", 
                        groupId = groupId,
...
datasetKey = pbi.executePBIOperation("getdatasetbyname",
                                    groupId = groupId,
                                    datasetName = datasetName)[0]["id"]
print(datasetKey)


# Insert the contents of the DataFrame in the PBI dataset table
pbi.executePBIOperation("postrows",
                        groupId = groupId,
...
dfInjectModified = dfInject.withColumn("rating", 
                                        dfInject.rating.cast("string"))
...
pbi.executePBIOperation("puttable",
                        groupId = groupId,
                        datasetKey = datasetKey,
                        tableNames = tableNames,
                        dataFrames = [dfInjectModified])
